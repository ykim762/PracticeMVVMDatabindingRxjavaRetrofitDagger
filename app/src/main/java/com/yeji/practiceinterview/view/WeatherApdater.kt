...
import com.yeji.practiceinterview.databinding.RowWeatherDailyAdapterBinding
...
class WeatherApdater(val mContext:Context):RecyclerView.Adapter<WeatherApdater.ViewHolder>() {
    ...
    inner class ViewHolder(var mBinding: RowWeatherDailyAdapterBinding):RecyclerView.ViewHolder(mBinding.root){
        fun bind(daily: Daily){

            mBinding.weather = daily
...
//        var view = LayoutInflater.from(mContext).inflate(R.layout.row_weather_daily_adapter, parent, false)
        var binding = DataBindingUtil.inflate<RowWeatherDailyAdapterBinding>(LayoutInflater.from(mContext), R.layout.row_weather_daily_adapter, parent, false)
